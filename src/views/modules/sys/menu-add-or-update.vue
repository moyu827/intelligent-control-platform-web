  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/menu/GET/menus'),
          method: 'get',
          params: this.$http.adornParams({
            type: 0
          })
        }).then(({data}) => {
          this.menuList = treeDataTranslate(data.rows, 'menuId')
        })
        if (id !== undefined && id !== 0) {
          this.$http({
            ...
            if (data.status === 'success') {
              this.dataForm.id = data.rows[0].menuId
              this.dataForm.type = data.rows[0].type
              this.dataForm.name = data.rows[0].name
              this.dataForm.parentId = data.rows[0].parentId
              this.dataForm.parentName = data.rows[0].parentName
              this.dataForm.url = data.rows[0].url
              this.dataForm.perms = data.rows[0].perms
              this.dataForm.orderNum = data.rows[0].orderNum
              this.dataForm.icon = data.rows[0].icon
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            if (id !== undefined && id !== 0) {
              this.$http({
                url: this.$http.adornUrl(`/sys/menu/PUT/menu`),
                method: 'put',
              ...
              this.$http({
                url: this.$http.adornUrl(`/sys/menu/POST/menu`),
                method: 'post',
                data: this.$http.adornData({
                  'type': this.dataForm.type,
                  'name': this.dataForm.name,
